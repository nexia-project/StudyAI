...
function getOpenAI() { return openrouter; }
...
router.post("/chat", checkFreeUsage, async (req, res) => {
  try {
    ...
    const HEAVY_TOOLS = new Set([
      "criar_slides", "criar_mapa_mental", "criar_infografico",
      "criar_resumo", "criar_plano_estudo", "criar_flashcards",
    ]);
    function buildAck(toolName: string, args: Record<string, any>): string {
      const t = args.topico || args.tema || "";
      const m = args.materia ? ` (${args.materia})` : "";
      const tBold = t ? ` sobre **${t}**` : "";
      switch (toolName) {
        case "criar_slides":       return `📚 Montando o livro digital${tBold}${m}... Pode levar alguns segundos — já aviso quando estiver pronto!`;
        case "criar_mapa_mental":  return `🗺️ Criando o mapa mental${tBold}${m}... Já organizo tudo e te mostro!`;
        case "criar_infografico":  return `📊 Gerando o infográfico${tBold}${m}... Aguenta um segundo!`;
        case "criar_resumo":       return `📝 Preparando o resumão${tBold}${m}... Já chego com tudo completo!`;
        case "criar_plano_estudo": return `📅 Montando seu plano de estudos personalizado... Trabalhando nisso agora!`;
        case "criar_flashcards":   return `🧠 Criando os flashcards${tBold}${m}... Rapidinho!`;
        default:                   return `⚙️ Executando isso pra você agora...`;
      }
    }
    ...
    // Acumula tool calls (chegam em deltas fragmentados) e encaminha texto direto
    ...
    for await (const chunk of stream1) {
      ...
      if (chunk.usage) { usageIn1 = chunk.usage.prompt_tokens; usageOut1 = chunk.usage.completion_tokens; }
      ...
      if (delta?.tool_calls) {
        for (const tc of delta.tool_calls as any[]) {
          ...
          const acc = toolCallsMap[tc.index];
          if (tc.id) acc.id = tc.id;
          if (tc.function?.name) acc.function.name += tc.function.name;
          if (tc.function?.arguments) acc.function.arguments += tc.function.arguments;

          // Assim que souber o nome da ferramenta pesada, emite ACK imediatamente
          if (!ackSent && acc.function.name && HEAVY_TOOLS.has(acc.function.name)) {
            // Tenta parsear args parciais para personalizar o ack
            let partialArgs: Record<string, any> = {};
            try { partialArgs = JSON.parse(acc.function.arguments || "{}"); } catch { /* ok */ }
            res.write(`data: ${JSON.stringify({ text: buildAck(acc.function.name, partialArgs) })}\n\n`);
            ackSent = true;
          }
        }
      }
    ...
    if (toolCalls.length > 0) {
      ...
      const toolResults: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [
        { role: "assistant", content: null, tool_calls: toolCalls } as any,
      ];
