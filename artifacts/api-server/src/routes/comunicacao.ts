...
router.post("/comunicacao/testar", async (req, res) => {
  try {
    ...
    return res.status(500).json({ erro: "Erro interno ao enviar.", detalhe: err.message });
  }
});
...
router.post("/comunicacao/disparar", async (req, res) => {
  try {
    ...
    return res.status(500).json({ erro: "Falha no envio. " + err.message });
  }
});
...
router.post("/comunicacao/check-triggers", async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ erro: "Erro no motor de triggers.", detalhe: err.message });
  }
});
...
router.post("/comunicacao/disparar-trigger", async (req, res) => {
  try {
    ...
    if (!email || !trigger_id) {
      return res.status(400).json({ erro: "email e trigger_id são obrigatórios." });
    }
    ...
    if (regraRows.length === 0) {
      return res.status(404).json({ erro: "Trigger não encontrado ou inativo." });
    }
    ...
    return res.json({ ok: true, trigger_id, emailId: data?.id });
  } catch (err: any) {
...
router.get("/comunicacao/regras", async (_req, res) => {
  try {
    ...
    `);
    return res.json({ regras: getRows(result) });
  } catch (err: any) {
    return res.status(500).json({ erro: err.message });
  }
});
...
router.get("/comunicacao/prefs/:user_id", async (req, res) => {
  try {
    ...
    if (rows.length === 0) {
      return res.json({ user_id: req.params.user_id, padrao: true });
    }
