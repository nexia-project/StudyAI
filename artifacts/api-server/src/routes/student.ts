...
router.post("/student/caderno/:id/process", async (req: Request, res: Response) => {
  ...
  try {
    const [note] = await db.select().from(cadernoNotesTable)
      .where(and(eq(cadernoNotesTable.id, req.params.id), eq(cadernoNotesTable.userId, req.userId!))).limit(1);
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [{
        role: "system",
        content: `Você é um especialista em educação. Processe as anotações do aluno e crie um pacote de aprendizagem.
Retorne JSON:
{
  "resumo": "resumo organizado em 3-4 parágrafos",
  "keyPoints": ["ponto 1", "ponto 2", "ponto 3", "ponto 4"],
  "mindMap": {"label": "tema", "emoji": "🎯", "children": [{"label": "subtópico", "emoji": "📖", "children": [{"label": "detalhe", "emoji": "🔹"}]}]},
  "flashcards": [{"front": "pergunta", "back": "resposta"}],
  "questoes": [{"text": "questão", "alternatives": ["A)...","B)...","C)...","D)..."], "correct": 0, "explanation": "explicação"}]
}`,
...
router.post("/student/activities/:id/submit", async (req: Request, res: Response) => {
  ...
  try {
    const [activity] = await db.select().from(activitiesTable).where(eq(activitiesTable.id, req.params.id)).limit(1);
