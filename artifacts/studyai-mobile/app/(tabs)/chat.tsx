...
import SlideCard, { type Slide } from "@/components/SlideCard";
...
export default function ChatScreen() {
  ...
  async function generateSlides() {
    ...
    if (!text || sending) return;
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
    const userMsg: Message = { id: Date.now().toString(), role: "user", content: `📊 Slides sobre: ${text}` };
    ...
    try {
      const res = await fetch(`${API_BASE}/api/chat/claude`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ message: text, type: "slides" }),
      });
      if (!res.ok) throw new Error("Erro ao gerar slides");
      const data = await res.json() as { response: string; slides?: { title: string; content: string[] }[]; model_used?: string; cached?: boolean };
      ...
      setMessages(prev => [...prev, { id: Date.now().toString() + "err", role: "assistant", content: "Não consegui gerar os slides agora. Tente novamente! 😅" }]);
    } finally {
      setSending(false);
    }
  }
  ...
  const renderItem = ({ item }: { item: Message }) => {
    ...
    return (
      <View style={[styles.bubble, isUser ? styles.userBubble : styles.aiBubble]}>
        ...
        <View style={[styles.bubbleInner, isUser ? styles.userInner : styles.aiInner, item.slides?.length ? { backgroundColor: "transparent", padding: 0, flex: 1 } : {}]}>
          {item.slides?.length ? (
            <SlideCard slides={item.slides} />
          ) : (
          ...
          {!isUser && item.cached && (
            <Text style={{ fontSize: 10, color: "#6ee7b7", marginTop: 4 }}>⚡ cache</Text>
          )}
        </View>
      </View>
    );
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior="padding" keyboardVerticalOffset={0}>
      ...
      <View style={[styles.inputRow, { paddingBottom: isWeb ? 34 : insets.bottom + 8 }]}>
        ...
        />
        <Pressable
          style={({ pressed }) => [styles.slidesBtn, (!input.trim() || sending) && styles.sendBtnDisabled, pressed && { opacity: 0.7 }]}
