...
function MapaMentalView({ map }: { map: MapaMental }) {
  const [openCat, setOpenCat] = useState<Set<number>>(new Set([0]));
  ...
  return (
    <div className="p-4 max-h-[600px] overflow-auto">
      <div className="flex justify-center mb-4">
        <div className="px-5 py-2.5 rounded-2xl font-black text-white text-base shadow-lg bg-gradient-to-r from-indigo-600 to-violet-600">
          {map.subject}
        </div>
      </div>
      <div className="space-y-2">
        {map.categories.map((cat, ci) => {
          ...
          const catOpen = openCat.has(ci);
          return (
            <div key={ci} className="rounded-xl border overflow-hidden" style={{ borderColor: color + "40" }}>
              <button
                onClick={() => setOpenCat(s => { const ns = new Set(s); ns.has(ci) ? ns.delete(ci) : ns.add(ci); return ns; })}
                className="w-full flex items-center gap-2.5 px-3 py-2 text-left"
                style={{ backgroundColor: color + "18" }}>
                <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: color }} />
                <span className="font-bold text-sm flex-1 text-slate-800">{cat.name}</span>
                <span className="text-xs text-slate-500">{cat.topics?.length ?? 0} tópicos</span>
                {catOpen ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
              </button>
              {catOpen && (
                <div className="px-2 pb-2 pt-1 space-y-1.5 bg-white/80">
                  {(cat.topics ?? []).map((topic, ti) => {
                    const key = `${ci}-${ti}`;
                    const topOpen = openTop.has(key);
                    return (
                      <div key={ti} className="rounded-lg border border-slate-100 overflow-hidden">
                        <button
                          onClick={() => setOpenTop(s => { const ns = new Set(s); ns.has(key) ? ns.delete(key) : ns.add(key); return ns; })}
                          className="w-full flex items-center gap-2 px-2.5 py-1.5 text-left bg-slate-50 hover:bg-slate-100">
                          <ChevronRight className={`w-3 h-3 text-slate-400 transition-transform ${topOpen ? "rotate-90" : ""}`} />
                          <span className="text-xs font-bold text-slate-700 flex-1">{topic.name}</span>
                          <span className="text-[10px] text-slate-400">{topic.subtopics?.length ?? 0}</span>
                        </button>
                        {topOpen && (
                            ...
                            {(topic.subtopics ?? []).map((sub, si) => (
                              <div key={si} className="flex items-start gap-2">
                                ...
                                <div className="flex-1">
                                  <p className="text-[11px] font-semibold text-slate-700">{sub.name}</p>
                                  {sub.detail && <p className="text-[10px] text-slate-500 leading-snug mt-0.5">{sub.detail}</p>}
                                </div>
                              </div>
                            ))}
                          </div>
