...
export function FlashcardsModal({
  ...
  const [partial, setPartial] = useState<Set<number>>(new Set());
  ...
  const markKnown = () => {
    ...
    setUnknown((prev) => { const n = new Set(prev); n.delete(cards[current].id); return n; });
    setPartial((prev) => { const n = new Set(prev); n.delete(cards[current].id); return n; });
    advance();
  };

  const markUnknown = () => {
    setUnknown((prev) => new Set([...prev, cards[current].id]));
    setKnown((prev) => { const n = new Set(prev); n.delete(cards[current].id); return n; });
    setPartial((prev) => { const n = new Set(prev); n.delete(cards[current].id); return n; });
    advance();
  };

  const markPartial = () => {
    setPartial((prev) => new Set([...prev, cards[current].id]));
    setKnown((prev) => { const n = new Set(prev); n.delete(cards[current].id); return n; });
    setUnknown((prev) => { const n = new Set(prev); n.delete(cards[current].id); return n; });
    advance();
  };
                ...
                >
                  ...
                  >
                    <div className="absolute top-4 left-4 right-4 flex items-center justify-between">
                      ...
                      <span className="text-[10px] text-gray-400 font-medium">{current + 1}/{cards.length}</span>
                    </div>
              ...
              <AnimatePresence>
                {flipped && (
                  ...
                  >
                    <button
                      onClick={markUnknown}
                      className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl bg-red-50 hover:bg-red-100 text-red-600 font-semibold transition-all hover:scale-[1.02] active:scale-95 text-sm"
                    >
                      <span>❌</span> Não sabia
                    </button>
                    <button
                      onClick={markPartial}
                      className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl bg-yellow-50 hover:bg-yellow-100 text-yellow-600 font-semibold transition-all hover:scale-[1.02] active:scale-95 text-sm"
                    >
                      <span>🤔</span> Parcial
                    </button>
                    <button
                      onClick={markKnown}
                      className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl bg-emerald-50 hover:bg-emerald-100 text-emerald-600 font-semibold transition-all hover:scale-[1.02] active:scale-95 text-sm"
                    >
                      <span>✅</span> Sabia!
                    </button>
                  </motion.div>
