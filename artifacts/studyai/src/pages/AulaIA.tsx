...
export default function AulaIA() {
  ...
  const playNarration = useCallback(async (texto: string, stepIdx: number) => {
    ...
    try {
      ...
      // ── Fallback: SpeechSynthesis pt-BR ──────────────────────────────────
      // /api/voice-tts (OpenAI TTS) is unavailable via the AI proxy.
      // Use the browser's built-in Speech Synthesis as a fallback so the
      // professor still narrates while the board writes letter-by-letter.
      if (myToken === fetchTokenRef.current && window.speechSynthesis) {
        ...
        // Pick a Portuguese voice when available (more natural).
        const voices = window.speechSynthesis.getVoices();
        const ptVoice = voices.find(v => v.lang.startsWith("pt")) ?? null;
        if (ptVoice) utter.voice = ptVoice;

        // When the narration finishes, mark the step as done — same behaviour
        // as the <audio> element's "ended" event.
        utter.onend = () => {
          if (myToken !== fetchTokenRef.current) return;
          setAudioEnded(true);
          setIsPlaying(false);
      ...
      // Release the loading gate so ChalkBoardCanvas starts drawing.
  ...
  const irParaEtapa = useCallback((idx: number, autoPlay = false) => {
    if (!aula) return;
    // Stop any current narration (HTML audio OR SpeechSynthesis fallback)
    window.speechSynthesis?.cancel();
    setEtapaAtual(idx);
    setBoardKey(k => k + 1);
    setBoardAllDone(false);
  ...
  const togglePlay = useCallback(() => {
    ...
    if (isPlaying) {
      audioRef.current?.pause();
      window.speechSynthesis?.cancel(); // also stop SpeechSynthesis fallback
      setIsPlaying(false);
    } else {
  ...
  const enviarPergunta = useCallback(async () => {
    ...
    if (isPlaying) { audioRef.current?.pause(); window.speechSynthesis?.cancel(); setIsPlaying(false); }
    try {
    ...
    try {
      ...
      if (r.ok) {
        ...
        setImagemGerada({
          ...
          topico: (etapa as any)?.titulo ?? aula?.titulo ?? topico,
        });
      }
    } catch { /* silent */ }
