...
function PannableSvg({ children, height = 600 }: { children: React.ReactNode; height?: number }) {
  ...
  const onTouchStart = (e: React.TouchEvent) => {
    if (e.touches.length === 1) {
      ...
      pinchRef.current = { d0: dist(e.touches[0] as any, e.touches[1] as any), s0: scale };
...
function MindMapNode({
  ...
  const w = isRoot ? 140 : isSubject ? 112 : 92;
  const h = isRoot ? 48 : isSubject ? 38 : 30;
  const clickable = node.hasContent && !isRoot;
  const opacity = node.source === "document" && !node.hasContent ? 0.45 : 1;
  const gradId = `rootGrad_${node.id}`;

  return (
    <g style={{ opacity }}>
      {/* Gradient def for root */}
      {isRoot && (
        <defs>
      ...
      {parentX !== undefined && parentY !== undefined && (
        <path
          ...
          strokeOpacity={node.source === "document" && !node.hasContent ? 0.25 : isSubject ? 0.6 : 0.45}
      ...
      >
        <rect
          width={w}
          height={h}
          rx={isRoot ? 18 : isSubject ? 12 : 10}
          fill={
            isRoot
              ? `url(#${gradId})`
              : isSubject
                ? (hovered && clickable ? node.color : "white")
                : (hovered && clickable ? node.color + "18" : node.source === "document" && !node.hasContent ? "#f3f4f6" : "white")
          }
          stroke={isRoot ? "none" : node.color}
          strokeWidth={isSubject ? 2.5 : 1.5}
          filter={isRoot ? "url(#shadowRoot)" : isSubject ? "url(#shadowCat)" : undefined}
        ...
        <text
          ...
          fontSize={isRoot ? 13 : isSubject ? 11 : 10}
          fontWeight={isRoot ? "800" : isSubject ? "bold" : "normal"}
          fill={
            isRoot
              ? "white"
              : isSubject
                ? (hovered && clickable ? "white" : node.color)
                : (hovered && clickable ? node.color : node.source === "document" && !node.hasContent ? "#9ca3af" : node.color)
          }
                  ...
                  <svg ref={svgRef} width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "100%", maxWidth: W }}>
                    <defs>
                      <filter id="shadowRoot" x="-25%" y="-25%" width="150%" height="150%">
                        <feDropShadow dx="0" dy="3" stdDeviation="5" floodOpacity="0.25" floodColor="#6366f1" />
                      </filter>
                      <filter id="shadowCat" x="-20%" y="-20%" width="140%" height="140%">
