...
const NAV: TeacherNavItem[] = [
  ...
  { id: "historico" as Section, label: "Histórico de Conteúdos", icon: History },
];
...
function SlideViewer({ slides }: { slides: SlideData[] }) {
  ...
  return (
    <div className="space-y-3">
      <div className={`rounded-2xl bg-gradient-to-br ${slideStyle[slide.tipo] ?? "from-slate-50 to-white border-gray-200"} border p-8 min-h-[260px] flex flex-col justify-center shadow-sm`}>
        <div className="text-4xl mb-4">{slide.emoji ?? "📄"}</div>
        {slide.tipo === "titulo" && (
          <>
            <h2 className="text-2xl font-black text-gray-900 mb-2">{slide.titulo}</h2>
            {slide.subtitulo && <p className="text-gray-600 text-lg">{slide.subtitulo}</p>}
          </>
        )}
        {(slide.tipo === "conteudo" || slide.tipo === "lista") && (
          <>
            <h3 className="text-xl font-bold text-gray-900 mb-4">{slide.titulo}</h3>
        ...
        {(slide.tipo === "exemplo" || slide.tipo === "destaque") && (
          <>
            <h3 className="text-xl font-bold text-gray-900 mb-3">{slide.titulo}</h3>
            <p className="text-gray-700 leading-relaxed">{slide.texto}</p>
          </>
        )}
        {slide.tipo === "quiz" && (
          <>
            ...
            <div className="bg-gray-100 rounded-xl p-4">
              <p className="text-gray-900 font-semibold">❓ {slide.pergunta}</p>
            </div>
          </>
        )}
      </div>
      <div className="flex items-center justify-between">
        <button onClick={() => setIdx(i => Math.max(0, i - 1))} disabled={idx === 0}
          className="p-2 rounded-xl bg-gray-50 hover:bg-gray-100 disabled:opacity-30 text-gray-900 transition-colors">
      ...
      <p className="text-center text-gray-400 text-xs">Slide {idx + 1} / {slides.length}</p>
    </div>
  );
}
                  ...
                  <div className="flex flex-col gap-2">
                    {child.children.map((sub, si) => (
                      <div key={si} className="flex items-center gap-2">
                        ...
                        <div className="px-3 py-1.5 rounded-lg bg-gray-100 border border-gray-200 text-gray-600 text-xs">
                          {sub.emoji} {sub.label}
                        </div>
                      </div>
                    ))}
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
        ...
        {content.keyPoints?.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-4">
            {content.keyPoints.map((kp, i) => (
              <span key={i} className="text-xs bg-gray-100 text-gray-600 px-3 py-1 rounded-full">{kp}</span>
            ))}
          </div>
        )}
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={tab} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          {tab === "mapa" && (
            <div className="rounded-2xl border border-gray-200 bg-white p-5">
              <h4 className="text-gray-500 text-xs mb-4 font-semibold uppercase tracking-wider">Mapa Mental</h4>
              {content.mindMap ? <MindMapViewer root={content.mindMap} /> : <p className="text-gray-400 text-sm text-center py-8">Mapa não disponível</p>}
            </div>
          )}
          {tab === "questoes" && (
                  ...
                  <div key={qi} className="rounded-2xl border border-gray-200 bg-white p-5">
                    ...
                    {show && (
                      <div className="bg-gray-50 rounded-xl px-4 py-3 text-xs text-gray-600">
                        <strong className="text-gray-900">Resposta:</strong> {q.explanation}
                      </div>
                    )}
                  </div>
                );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-violet-50/40 flex">
      ...
      <main className="flex-1 ml-16 lg:ml-64 flex flex-col min-h-screen">
        <header className="sticky top-0 z-10 bg-white/80 backdrop-blur-xl border-b border-gray-200/80 px-6 py-4 flex items-center gap-4">
        ...
        <div className="flex-1 p-4 lg:p-8 max-w-[1400px] w-full mx-auto">
          <AnimatePresence mode="wait">
            <motion.div key={section} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25 }}>
              ...
              {section === "estudio" && (
                <div className="max-w-5xl mx-auto"><EstudioIA variant="light" title="Estúdio Visual IA" /></div>
              )}
              ...
              {section === "historico" && <HistoricoSection apiFetch={apiFetch} />}
            </motion.div>
          </AnimatePresence>
        </div>
      </main>
...
function DashboardSection({ apiFetch, onNavigate }: { apiFetch: (u: string, o?: RequestInit) => Promise<Response>; onNavigate: (s: Section) => void }) {
  ...
  if (loading) return (
    <div className="space-y-6">
      {[...Array(3)].map((_, i) => (
        <div key={i} className="rounded-3xl border border-gray-200 bg-white p-5 animate-pulse h-32" />
      ))}
    </div>
  );
  ...
  const engagementLabel = (rate: number) =>
    ...
    rate >= 40 ? { label: "Médio", color: "text-amber-700 bg-amber-50 border-amber-200" } :
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <motion.div key={k.label}
            ...
            <p className="text-gray-500 text-xs font-semibold mt-1.5">{k.label}</p>
          </motion.div>
        ))}
      </div>
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[11px] font-black tracking-widest text-amber-600 uppercase">Geradores AI</span>
            </div>
            <h3 className="font-black text-gray-900 text-xl tracking-tight">Crie em segundos com Claude</h3>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            <motion.button key={btn.label}
              initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.05, duration: 0.35 }}
              onClick={() => onNavigate(btn.s)}
              className={`relative overflow-hidden flex flex-col items-start gap-3 bg-gradient-to-br ${btn.bg} border ${btn.border} rounded-2xl p-5 text-left transition-all group hover:shadow-lg hover:-translate-y-0.5`}>
              <div className={`w-11 h-11 rounded-2xl bg-gradient-to-br ${btn.gradient} flex items-center justify-center shadow-md shadow-black/10 group-hover:scale-110 transition-transform`}>
                <btn.icon className={`w-5 h-5 ${btn.iconColor}`} strokeWidth={2.5} />
              </div>
              <div>
                <p className="text-gray-900 text-sm font-black leading-tight tracking-tight">{btn.label}</p>
                <p className="text-gray-600 text-xs mt-1 leading-snug">{btn.desc}</p>
              </div>
              <ChevronRight className="absolute top-4 right-4 w-4 h-4 text-gray-400 group-hover:text-gray-700 group-hover:translate-x-0.5 transition-all" />
            </motion.button>
          ))}
        </div>
      </motion.div>
                ...
                return (
                  <div key={t.id} className="rounded-2xl border border-gray-200 bg-gradient-to-br from-gray-50 to-white p-4 hover:border-violet-300 hover:shadow-md transition-all group">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ml-2 flex-shrink-0 ${eng.color}`}>{eng.label}</span>
                    </div>
                    <div className="flex items-center gap-3 text-xs text-gray-600 mb-3">
                      <span className="flex items-center gap-1 font-semibold"><Users className="w-3.5 h-3.5 text-gray-400" />{t.studentCount} alunos</span>
                    </div>
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => onNavigate("atividades")}
                        className="flex-1 py-2 rounded-xl bg-white hover:bg-gray-50 border border-gray-200 hover:border-gray-300 text-gray-700 text-xs font-bold transition-all">
                        Atividade
                      </button>
                    </div>
                  </div>
          ...
          <motion.div {...fadeUp} className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-8 h-8 rounded-xl bg-amber-50 flex items-center justify-center">
                <Bell className="w-4 h-4 text-amber-600" strokeWidth={2.5} />
              </div>
              <h3 className="font-black text-gray-900 text-sm tracking-tight">Alertas</h3>
            </div>
            <div className="space-y-2">
              {data.alerts.length === 0 ? (
                <div className="text-center py-4">
                  <CheckCircle2 className="w-6 h-6 text-emerald-500 mx-auto mb-1.5" />
                  <p className="text-gray-500 text-xs font-semibold">Tudo em ordem!</p>
                </div>
              ) : data.alerts.map((a, i) => (
                <div key={i} className={`flex items-start gap-2 rounded-xl px-3 py-2.5 text-xs border ${
            ...
            {data.weeklyChart.every(w => w.acertos === 0) ? (
              ...
              <ResponsiveContainer width="100%" height={120}>
                <LineChart data={data.weeklyChart} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
                  <XAxis dataKey="week" tick={{ fill: "#9ca3af", fontSize: 10, fontWeight: 600 }} axisLine={false} tickLine={false} />
                  <Tooltip contentStyle={{ background: "white", border: "1px solid #e5e7eb", borderRadius: "12px", color: "#111827", fontSize: "11px", boxShadow: "0 4px 16px rgba(0,0,0,0.08)" }} />
                  <Line type="monotone" dataKey="acertos" stroke="#10b981" strokeWidth={2.5} dot={false} name="Acertos %" />
                  <Line type="monotone" dataKey="participacao" stroke="#6366f1" strokeWidth={2} dot={false} name="Participação" />
                </LineChart>
              </ResponsiveContainer>
            )}
          </motion.div>
        ...
        {atividades.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {atividades.map((a: any) => {
              ...
              return (
                <div key={a.id} className="flex items-center gap-3 py-2.5 px-3 rounded-2xl hover:bg-gray-50 border border-transparent hover:border-gray-200 transition-all group cursor-pointer">
                  <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold border flex-shrink-0 ${st.color}`}>{st.label}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-gray-900 text-sm font-bold truncate">{a.title}</p>
                    <p className="text-gray-500 text-xs">{a.turmaNome ?? "Turma"} • {daysAgo === 0 ? "Hoje" : daysAgo === 1 ? "Ontem" : `${daysAgo}d atrás`}</p>
                  </div>
                  <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full flex-shrink-0 ${a.is_published ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-gray-100 text-gray-600 border border-gray-200"}`}>
        ...
        <motion.div {...fadeUp} className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-5">
            <div className="w-9 h-9 rounded-2xl bg-rose-50 flex items-center justify-center">
              <Target className="w-4.5 h-4.5 text-rose-600" strokeWidth={2.5} />
            </div>
            <div>
              <h3 className="font-black text-gray-900 text-base tracking-tight">Desempenho por Matéria</h3>
              <p className="text-gray-500 text-xs">Mapa de calor agregado das suas turmas</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2.5">
            {data.heatMap.map(h => (
              <div key={h.materia} className={`rounded-2xl px-4 py-3 text-center border ${SCORE_COLOR(h.score)}`}>
                <p className="text-xs font-bold opacity-80">{h.materia}</p>
                <p className="font-black text-xl tracking-tight">{h.score}%</p>
              </div>
            ))}
          </div>
        </motion.div>
          ...
          {turmas.map((t, i) => (
            <motion.div key={t.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <button onClick={() => deleteTurma(t.id)} className="p-1.5 text-gray-300 hover:text-red-600 rounded-lg transition-colors ml-2"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
              <div className="flex items-center gap-1.5 text-gray-400 text-xs mb-3"><Users className="w-3.5 h-3.5" />{t.studentCount} alunos</div>
    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20 border border-gray-200 rounded-2xl">
          <UserCircle className="w-12 h-12 text-gray-200 mx-auto mb-4" />
          ...
          <table className="w-full text-xs">
            ...
            <tbody>{filtered.map((s, i) => (
              <tr key={s.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-2.5 text-gray-500 hidden sm:table-cell">{s.turma}</td>
                <td className="px-4 py-2.5 text-center">
                  <span className={`px-2 py-0.5 rounded-full font-bold ${s.performance >= 70 ? "bg-emerald-100 text-emerald-700" : s.performance >= 40 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"}`}>{s.performance}%</span>
                </td>
                <td className="px-4 py-2.5 text-center hidden md:table-cell">
                  <span className={`font-semibold ${s.engagement === "Alto" ? "text-emerald-600" : s.engagement === "Médio" ? "text-amber-600" : "text-red-600"}`}>{s.engagement}</span>
                </td>
              </tr>
            ))}</tbody>
          </table>
...
function ConteudosSection({ apiFetch }: { apiFetch: (u: string, o?: RequestInit) => Promise<Response> }) {
  ...
  async function gerarPremium() {
    if (!topico.trim()) { alert("Informe o tópico antes."); return; }
    setPremiumLoading(true);
    try {
      const r = await apiFetch("/api/teacher/material-premium", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ topico, materia: "Geral", nivel, contexto: conteudo }),
      });
      ...
      if (d.ok && d.html) {
        ...
        alert(d.error || "Erro ao gerar material premium");
      }
    } catch (e: any) {
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-gray-500 text-xs">Gere aulas, slides, mapas mentais e questões automaticamente</p>
          </div>
        </div>
        ...
        <form onSubmit={generate} className="space-y-4">
          ...
          <button type="button" onClick={gerarPremium} disabled={premiumLoading || !topico.trim()}
            className="w-full py-3 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 disabled:opacity-50 text-white font-bold text-sm flex items-center justify-center gap-2 transition-colors">
            {premiumLoading
              ? <><Loader2 className="w-4 h-4 animate-spin" />Montando material editorial premium (gráficos + cards + quiz)...</>
              : <><Sparkles className="w-4 h-4" />✨ Material Premium HTML (visual rico, pronto pra impressão)</>}
          </button>
          <p className="text-[11px] text-gray-500 text-center">O material premium abre numa nova aba — apresentação editorial completa com gráficos, ícones, cards e quiz interativo. Salvo em "Meus Conteúdos".</p>
        </form>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-gray-500 text-xs">Busque qualquer tema e receba um pacote completo de aprendizagem</p>
          </div>
        </div>
        ...
        {!result && !loading && (
          <div className="mt-4">
            <p className="text-gray-400 text-xs mb-3">Sugestões de busca</p>
            <div className="flex flex-wrap gap-2">
              {SUGGESTIONS.map(s => (
...
function GerarProvaSection({ apiFetch }: { apiFetch: (u: string, o?: RequestInit) => Promise<Response> }) {
  ...
  // Visual style themes
  const theme = {
    enem: { card: "bg-gray-50 border-gray-200", num: "bg-slate-700 text-white", alt: "bg-gray-50 border-gray-200 hover:bg-gray-100 text-gray-600" },
    infantil: { card: "bg-gradient-to-br from-pink-900/30 to-purple-900/30 border-pink-200", num: "bg-gradient-to-br from-pink-500 to-purple-500 text-white", alt: "bg-gray-100 border-gray-300 hover:bg-pink-50 text-gray-700" },
    ...
    aventura: { card: "bg-gradient-to-br from-amber-900/20 to-orange-900/20 border-amber-600/20", num: "bg-gradient-to-br from-amber-500 to-orange-500 text-white", alt: "bg-amber-50 border-amber-600/20 hover:bg-amber-600/30 text-amber-100" },
  }[visualStyle];
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-gray-500 text-xs">Provas com IA — Modo Mundo, ilustrações e estilos visuais</p>
          </div>
        </div>

        {/* Exam mode */}
        <div className="mb-5">
          <p className="text-gray-500 text-xs font-semibold mb-2 uppercase tracking-wider">Modo da Prova</p>
        ...
        {/* Visual style */}
        <div className="mb-5">
          <p className="text-gray-500 text-xs font-semibold mb-2 uppercase tracking-wider">Estilo Visual</p>
        ...
        <form onSubmit={generate} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <div className="sm:col-span-2 lg:col-span-2">
            <label className="text-gray-500 text-xs mb-1.5 block">Questões: {form.quantidade}</label>
      ...
      {exam && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          {worldMode && exam.story && (
            <div className="rounded-2xl border border-amber-300 bg-gradient-to-br from-amber-900/30 to-orange-900/20 p-6 mb-5">
              <div className="flex items-center gap-3 mb-4">
                <span className="text-4xl">{exam.story.emoji ?? "🗺️"}</span>
                <div>
                  <h2 className="text-xl font-black text-gray-900">{exam.title}</h2>
                  <p className="text-amber-700/80 text-sm font-semibold">{exam.story.missao}</p>
                </div>
              </div>
              <div className="grid sm:grid-cols-3 gap-3 text-xs">
                <div className="bg-black/20 rounded-xl p-3">
                  <p className="text-amber-600/60 mb-1 font-semibold">🌍 Cenário</p>
                  <p className="text-amber-100/80">{exam.story.cenario}</p>
                </div>
                <div className="bg-black/20 rounded-xl p-3">
                  <p className="text-amber-600/60 mb-1 font-semibold">🧙 Guia</p>
                  <p className="text-amber-100/80">{exam.story.personagem}</p>
                </div>
                <div className="bg-black/20 rounded-xl p-3">
                  <p className="text-amber-600/60 mb-1 font-semibold">🏆 Objetivo</p>
          ...
          {/* Classic header */}
          {!worldMode && (
            <div className={`rounded-2xl border p-5 mb-5 ${visualStyle === "infantil" ? "bg-gradient-to-br from-pink-800/30 to-purple-800/30 border-pink-300" : visualStyle === "tecnico" ? "bg-zinc-900/80 border-zinc-700/50" : "border-gray-200 bg-white"}`}>
              <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                {showResult && (
                  <div className={`text-center px-5 py-2 rounded-xl ${score >= exam.questions.length * 0.7 ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                ...
                <form onSubmit={applyToTurma} className="space-y-3">
                  <div>
                    <label className="text-gray-500 text-xs mb-1.5 block">Título da Atividade</label>
                  ...
                  <div>
                    <label className="text-gray-500 text-xs mb-1.5 block">Turma</label>
                  ...
                  <div>
                    <label className="text-gray-500 text-xs mb-1.5 block">Prazo</label>
            ...
            {(exam.questions ?? []).map((q, qi) => {
              ...
              return (
                <motion.div key={qi} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: qi * 0.06 }}
                  className={`rounded-2xl border p-5 transition-all ${showResult ? (isCorrect ? "border-emerald-500/40 bg-emerald-50" : isWrong ? "border-red-500/40 bg-red-50" : theme.card) : theme.card}`}>
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      {worldMode && q.desafio && (
                        <div className="bg-amber-50 border border-amber-200 rounded-xl px-3 py-2 mb-3 text-xs text-amber-700 italic">
                          ⚡ {q.desafio}
                        </div>
                      )}
                      ...
                      {showResult && (
                        <div className="mt-3 bg-gray-50 rounded-xl px-3 py-2.5 text-xs text-gray-600">
                          <strong className="text-gray-900">Explicação:</strong> {q.explanation}
                        </div>
                      )}
                    </div>
                  </div>
          ...
          {worldMode && showResult && (
            <div className={`text-center p-6 rounded-2xl border ${score >= exam.questions.length * 0.7 ? "bg-emerald-50 border-emerald-300" : "bg-amber-50 border-amber-300"}`}>
              <div className="text-4xl mb-2">{score >= exam.questions.length * 0.7 ? "🏆" : "⚔️"}</div>
              <p className="text-gray-900 font-black text-xl">{score}/{exam.questions.length} desafios concluídos!</p>
              <p className={`text-sm mt-1 ${score >= exam.questions.length * 0.7 ? "text-emerald-700" : "text-amber-700"}`}>
...
function RelatoriosSection({ apiFetch }: { apiFetch: (u: string, o?: RequestInit) => Promise<Response> }) {
  ...
  return (
    <div className="space-y-5">
      {/* Export buttons */}
      <div className="flex items-center justify-between">
        <h2 className="text-gray-900 font-black text-base">Relatório Geral</h2>
        <div className="flex gap-2">
          <button onClick={exportCsv} disabled={exportingCsv}
            className="flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-600 text-xs font-bold transition-colors disabled:opacity-50">
      ...
      {data.heatMap.length > 0 && (
        <div className="rounded-2xl border border-gray-200 bg-white p-5">
          <h3 className="font-bold text-gray-900 text-sm mb-5">Desempenho por Matéria</h3>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={data.heatMap} margin={{ top: 5, right: 10, left: -25, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
              <XAxis dataKey="materia" tick={{ fill: "#6b7280", fontSize: 10, fontWeight: 600 }} axisLine={false} tickLine={false} />
              <YAxis domain={[0, 100]} tick={{ fill: "#6b7280", fontSize: 10, fontWeight: 600 }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ background: "white", border: "1px solid #e5e7eb", borderRadius: "12px", color: "#111827", fontSize: "12px", boxShadow: "0 4px 16px rgba(0,0,0,0.08)" }} />
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5">
        <h3 className="font-bold text-gray-900 text-sm mb-5">Evolução Semanal</h3>
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={data.weeklyChart} margin={{ top: 5, right: 10, left: -25, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
            <XAxis dataKey="week" tick={{ fill: "#6b7280", fontSize: 10, fontWeight: 600 }} axisLine={false} tickLine={false} />
            <YAxis domain={[0, 100]} tick={{ fill: "#6b7280", fontSize: 10, fontWeight: 600 }} axisLine={false} tickLine={false} />
            <Tooltip contentStyle={{ background: "white", border: "1px solid #e5e7eb", borderRadius: "12px", color: "#111827", fontSize: "12px", boxShadow: "0 4px 16px rgba(0,0,0,0.08)" }} />
            <Legend wrapperStyle={{ fontSize: "10px", color: "#6b7280", fontWeight: 600 }} />
      ...
      {data.alerts.length > 0 && (
        <div className="rounded-2xl border border-gray-200 bg-white p-5">
          <h3 className="font-bold text-gray-900 text-sm mb-4 flex items-center gap-2"><Bell className="w-4 h-4 text-amber-600" />Alertas do Sistema</h3>
          <div className="space-y-2">
            {data.alerts.map((a, i) => (
              <div key={i} className={`flex items-start gap-2.5 rounded-xl p-3 ${a.severity === "warning" ? "bg-amber-50 border border-amber-200" : "bg-emerald-50 border border-emerald-200"}`}>
                {a.severity === "warning" ? <AlertTriangle className="w-4 h-4 text-amber-600 mt-0.5 flex-shrink-0" /> : <CheckCircle2 className="w-4 h-4 text-emerald-600 mt-0.5 flex-shrink-0" />}
                <p className={`text-xs ${a.severity === "warning" ? "text-amber-700" : "text-emerald-700"}`}>{a.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <p className="text-gray-300 text-[11px] text-center">
        💡 Para relatório detalhado por turma, acesse a turma e use a aba <strong className="text-gray-500">Desempenho</strong>
      </p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h2 className="font-black text-gray-900 text-lg">Banco de Questões</h2>
          <p className="text-gray-500 text-xs">{questions.length} questão(ões) cadastrada(s)</p>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <form onSubmit={generateAI} className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-gray-500 text-xs mb-1.5 block">Tema / Assunto *</label>
                <input required value={aiForm.tema} onChange={e => setAiForm(p => ({ ...p, tema: e.target.value }))}
                  placeholder="Ex: Segunda Guerra Mundial" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900" />
              </div>
              <div>
                <label className="text-gray-500 text-xs mb-1.5 block">Tipo de Questão</label>
              ...
              <div>
                <label className="text-gray-500 text-xs mb-1.5 block">Quantidade</label>
        ...
        <div className="rounded-2xl border border-violet-300 bg-violet-600/5 p-5">
          ...
          <form onSubmit={saveQuestion} className="space-y-4">
            <div className="grid sm:grid-cols-3 gap-3">
              ...
              <div>
                <label className="text-gray-500 text-xs mb-1.5 block">Tema</label>
                <input value={form.tema} onChange={e => setForm(p => ({ ...p, tema: e.target.value }))} placeholder="Ex: Funções do 1º grau"
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900" />
              </div>
            </div>
            <div>
              <label className="text-gray-500 text-xs mb-1.5 block">Contexto (texto-base, opcional)</label>
            ...
            <div>
              <label className="text-gray-500 text-xs mb-1.5 block">Enunciado *</label>
              <textarea required value={form.text} onChange={e => setForm(p => ({ ...p, text: e.target.value }))} rows={3}
                className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 resize-none" />
            </div>
            <div className="space-y-2">
              <label className="text-gray-500 text-xs">Alternativas (letra verde = gabarito)</label>
              {form.alternatives.map((alt: string, i: number) => (
                <div key={i} className="flex items-center gap-2">
                  <button type="button" onClick={() => setForm(p => ({ ...p, correct: i }))}
                    className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0 border transition-colors ${form.correct === i ? "bg-emerald-500 border-emerald-400 text-white" : "bg-gray-50 border-gray-200 text-gray-500"}`}>
            ...
            <div>
              <label className="text-gray-500 text-xs mb-1.5 block">Explicação do gabarito</label>
      ...
      {showApplyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-md shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-gray-900">Criar Atividade do Banco</h3>
              <button onClick={() => setShowApplyModal(false)} className="text-gray-500 hover:text-gray-900"><X className="w-4 h-4" /></button>
            </div>
            <p className="text-gray-500 text-xs mb-4">{selected.size} questão(ões) selecionada(s) serão incluídas nesta atividade.</p>
            <form onSubmit={applyFromBank} className="space-y-3">
              <div>
                <label className="text-gray-500 text-xs mb-1.5 block">Título *</label>
                <input required value={applyForm.title} onChange={e => setApplyForm(f => ({ ...f, title: e.target.value }))}
                  placeholder="Ex: Prova de Revisão" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900" />
              </div>
              <div>
                <label className="text-gray-500 text-xs mb-1.5 block">Turma (opcional)</label>
                <select value={applyForm.turmaId} onChange={e => setApplyForm(f => ({ ...f, turmaId: e.target.value }))}
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900">
                  <option value="">Todas as turmas</option>
                  {turmas.map((t: any) => <option key={t.id} value={t.id}>{t.name}</option>)}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="button" onClick={() => setShowApplyModal(false)} className="px-4 py-2.5 rounded-xl bg-gray-50 text-gray-600 text-sm font-bold">Cancelar</button>
              </div>
            </form>
          </motion.div>
        </div>
                        ...
                        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                          {(q.alternatives as string[]).filter(Boolean).map((alt: string, i: number) => (
                            <div key={i} onClick={e => e.stopPropagation()}
                              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs ${q.correct === i ? "bg-emerald-50 text-emerald-700" : "bg-gray-50 text-gray-500"}`}>
                      ...
                      {q.tipo === "vf" && (
                        <div className="mt-2 flex gap-2">
                          {["Verdadeiro","Falso"].map((opt, i) => (
                            <span key={i} className={`px-3 py-1 rounded-lg text-xs ${q.correct === i ? "bg-emerald-50 text-emerald-700 font-bold" : "bg-gray-50 text-gray-500"}`}>{opt}</span>
                          ))}
                        </div>
                      )}
                      {q.tipo === "discursiva" && q.explanation && (
                        <div className="mt-2 text-xs text-gray-500 bg-gray-50 rounded-lg p-2">
                          <span className="font-bold text-gray-600">Gabarito: </span>{q.explanation}
                        </div>
                      )}
                    </div>
                    <button onClick={e => { e.stopPropagation(); deleteQuestion(q.id); }} className="p-1.5 text-gray-300 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors flex-shrink-0">
...
function AtividadesSection({ apiFetch }: { apiFetch: (u: string, o?: RequestInit) => Promise<Response> }) {
  ...
  if (selectedSub && selectedActivity) {
    ...
    return (
      <div className="space-y-5 max-w-4xl">
        <div className="flex items-center gap-3">
          <button onClick={() => setSelectedSub(null)} className="p-2 rounded-xl bg-gray-50 hover:bg-gray-100 text-gray-600 hover:text-gray-900 transition-colors">
            <ChevronLeft className="w-4 h-4" />
          </button>
          <div className="flex-1">
            <h2 className="font-black text-gray-900 text-sm">{selectedSub.student_name || selectedSub.first_name || selectedSub.student_id?.slice(0, 8)}</h2>
            <p className="text-gray-500 text-xs">{selectedActivity.title} • Enviado {new Date(selectedSub.submitted_at).toLocaleDateString("pt-BR")}</p>
          </div>
              ...
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {Object.entries(answers).map(([qi, ans]: [string, any]) => (
                  <div key={qi} className="bg-gray-50 rounded-xl p-3">
                    <p className="text-gray-500 text-[10px] mb-1">Questão {Number(qi) + 1}</p>
                    <p className="text-gray-600 text-xs">{typeof ans === "number" ? `Alternativa ${String.fromCharCode(65 + ans)}` : ans}</p>
                  </div>
                ))}
              </div>
            )}
            ...
            {!ai ? (
              ...
              <div className="space-y-3 max-h-80 overflow-y-auto">
                {(ai.competencias ?? []).map((c: any) => (
                  <div key={c.numero} className="bg-gray-50 rounded-xl p-3">
                    <div className="flex items-center justify-between mb-1">
                      <p className="text-gray-600 text-xs font-semibold">{c.nome}</p>
                      <span className={`text-xs font-black px-2 py-0.5 rounded-full ${c.nota >= 160 ? "bg-emerald-100 text-emerald-700" : c.nota >= 80 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"}`}>
                        {c.nota}/200
                      </span>
                    </div>
              ...
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {(ai.feedbacks ?? []).map((fb: any) => (
                  <div key={fb.qi} className="bg-gray-50 rounded-xl p-3">
                    <div className="flex items-center justify-between mb-1">
                      <p className="text-gray-500 text-[10px]">Q{fb.qi + 1}</p>
                      <span className="text-emerald-700 text-[10px] font-bold">{fb.pontos}/10</span>
                    </div>
                    <p className="text-gray-600 text-xs">{fb.feedbackAluno || fb.feedback}</p>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
          <h3 className="font-bold text-gray-900 text-sm mb-4 flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-600" /> Nota e Feedback do Professor
          </h3>
          {selectedSub.correction_status === "corrigido" ? (
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <span className="text-emerald-700 font-black text-2xl">{selectedSub.teacher_score}</span>
            ...
            <div className="space-y-3">
              <div className="grid sm:grid-cols-4 gap-3">
                <div>
                  <label className="text-gray-500 text-xs block mb-1.5">Nota (0–100)</label>
                  <input type="number" min={0} max={100} value={corrForm.teacher_score}
                    ...
                    className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-gray-900 text-sm focus:border-emerald-500 outline-none" />
                </div>
                <div className="sm:col-span-3">
                  <label className="text-gray-500 text-xs block mb-1.5">Feedback para o Aluno</label>
  ...
  if (selectedActivity) return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        <button onClick={() => { setSelectedActivity(null); setSubmissions([]); }} className="p-2 rounded-xl bg-gray-50 hover:bg-gray-100 text-gray-600 hover:text-gray-900 transition-colors">
          <ChevronLeft className="w-4 h-4" />
        </button>
        <div className="flex-1">
          <h2 className="font-black text-gray-900">{selectedActivity.title}</h2>
          <div className="flex gap-2 mt-1 flex-wrap">
            <span className="text-gray-500 text-xs">{submissions.length} entrega(s)</span>
            {(selectedActivity.isPublished || selectedActivity.is_published)
              ? <span className="text-xs bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full font-semibold">Publicada</span>
              : <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full font-semibold">Rascunho</span>
            }
          </div>
        </div>
        <button onClick={() => togglePublish(selectedActivity)}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
            selectedActivity.isPublished || selectedActivity.is_published
              ? "bg-amber-50 border border-amber-200 text-amber-700 hover:bg-amber-500/25"
              : "bg-emerald-600/20 border border-emerald-200 text-emerald-700 hover:bg-emerald-600"
          }`}>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[500px]">
                <thead><tr className="border-b border-gray-200">
                  <th className="text-left px-4 py-3 text-gray-500 text-xs font-bold">Aluno</th>
                  <th className="text-center px-4 py-3 text-gray-500 text-xs font-bold">Nota</th>
                  <th className="text-center px-4 py-3 text-gray-500 text-xs font-bold hidden sm:table-cell">Status</th>
                  <th className="text-center px-4 py-3 text-gray-500 text-xs font-bold hidden md:table-cell">Enviado</th>
                  <th className="text-right px-4 py-3 text-gray-500 text-xs font-bold">Ação</th>
                </tr></thead>
                <tbody>
                  {submissions.map((s: any) => {
                    ...
                    return (
                      <tr key={s.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3 text-gray-900 font-medium">{s.student_name || s.first_name || s.student_id?.slice(0,8)}</td>
                        <td className="px-4 py-3 text-center">
                          {pct != null ? (
                            <span className={`px-2 py-1 rounded-lg text-xs font-black ${pct >= 70 ? "bg-emerald-100 text-emerald-700" : pct >= 50 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"}`}>
                              {pct}
                            </span>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="font-black text-gray-900 text-lg">Atividades</h2>
      ...
      {showForm && (
        <div className="rounded-2xl border border-violet-300 bg-violet-600/5 p-5">
          ...
          <form onSubmit={saveActivity} className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-3">
              <div>
                ...
                <input required value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
                  placeholder="Ex: Prova de Matemática" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900" />
              </div>
              <div>
            ...
            <div>
              <label className="text-gray-500 text-xs mb-1.5 block">Descrição</label>
            ...
            <div className="space-y-3">
              ...
              {questoes.map((q, qi) => (
                <div key={qi} className="rounded-xl border border-gray-200 bg-gray-50 p-4 space-y-2.5">
                  <div className="flex items-center gap-2">
                    <span className="text-gray-400 text-xs font-bold">{qi + 1}.</span>
                    <input value={q.text} onChange={e => { const qs = [...questoes]; qs[qi] = { ...qs[qi], text: e.target.value }; setQuestoes(qs); }}
                      placeholder="Enunciado" className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-900" />
                    {qi > 0 && <button type="button" onClick={() => setQuestoes(q => q.filter((_, i) => i !== qi))} className="text-gray-300 hover:text-red-600"><X className="w-3.5 h-3.5" /></button>}
                  </div>
                  {q.alternatives.map((alt: string, ai: number) => (
                    <div key={ai} className="flex items-center gap-2">
                      <button type="button" onClick={() => { const qs = [...questoes]; qs[qi] = { ...qs[qi], correct: ai }; setQuestoes(qs); }}
                        className={`w-6 h-6 rounded-lg text-[10px] font-black flex-shrink-0 border transition-colors ${q.correct === ai ? "bg-emerald-500 border-emerald-400 text-white" : "bg-gray-50 border-gray-200 text-gray-400"}`}>
                      ...
                      <input value={alt} onChange={e => { const qs = [...questoes]; const alts = [...qs[qi].alternatives]; alts[ai] = e.target.value; qs[qi] = { ...qs[qi], alternatives: alts }; setQuestoes(qs); }}
                        placeholder={`Alt. ${String.fromCharCode(65 + ai)}`} className="flex-1 bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5 text-xs text-gray-900" />
                    </div>
                  ))}
                </div>
              ))}
        ...
        : activities.length === 0 ? (
          ...
          <div className="space-y-3">
            {activities.map((a: any) => {
              ...
              return (
                <div key={a.id} className="rounded-2xl border border-gray-200 bg-white p-4 hover:border-gray-200 transition-colors">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        {(a.dueDate || a.due_date) && <span className="text-xs text-gray-400 flex items-center gap-1"><Calendar className="w-3 h-3" />{new Date(a.dueDate || a.due_date).toLocaleDateString("pt-BR")}</span>}
                      </div>
                      <h3 className="font-bold text-gray-900">{a.title}</h3>
                      {a.description && <p className="text-gray-500 text-xs mt-0.5 truncate">{a.description}</p>}
                      <div className="flex items-center gap-4 mt-2">
                        <span className="text-gray-400 text-xs">{(a.content as any)?.questions?.length ?? 0} questões</span>
                        <span className="text-gray-400 text-xs">{a.submissionCount ?? 0} entregas</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <button onClick={() => togglePublish(a)} title={pub ? "Despublicar" : "Publicar"}
                        className={`p-2 rounded-xl transition-colors ${pub ? "bg-amber-50 text-amber-600 hover:bg-amber-100" : "bg-emerald-50 text-emerald-600 hover:bg-emerald-100"}`}>
      ...
      <div className="flex items-center justify-between">
        ...
        {step === "result" && (
          <button onClick={() => { setStep("form"); setPlano(null); setSavedId(null); }}
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gray-100 hover:bg-gray-100 text-gray-600 hover:text-gray-900 text-sm font-bold transition-all">
        ...
        <div className="grid lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 rounded-2xl border border-gray-200 bg-white p-5">
            <form onSubmit={handleGenerate} className="space-y-4">
              {error && (
                <div className="rounded-xl bg-red-50 border border-red-200 text-red-700 text-xs px-3 py-2.5 flex items-center gap-2">
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex gap-2 flex-wrap">
                <button onClick={() => handleAjuste("simplificar")} disabled={generating}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-50 hover:bg-amber-500/25 border border-amber-200 text-amber-700 text-xs font-bold transition-all disabled:opacity-50">
                ...
                {!savedId && (
                  <button onClick={handleSave} disabled={saving}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-500/25 border border-emerald-200 text-emerald-700 text-xs font-bold transition-all disabled:opacity-50">
          ...
          <div className="grid lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 space-y-3">
                ...
                <div key={block.key} className="rounded-2xl border border-gray-200 bg-white overflow-hidden">
                  <button onClick={() => toggleSection(block.key)}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-400 text-xs">— {block.data.duracao}</span>
                    </div>
                    <ChevronRight className={`w-4 h-4 text-gray-400 transition-transform ${openSections[block.key] ? "rotate-90" : ""}`} />
                  </button>
                  {openSections[block.key] && (
                    <div className="px-5 pb-4 space-y-3">
                      {block.items.map((item, i) => (
                        <div key={i}>
                          <p className="text-gray-500 text-[10px] font-bold uppercase tracking-wide mb-1">{item.l}</p>
                          <p className="text-gray-600 text-xs leading-relaxed">{item.v}</p>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {plano.tarefa_casa && (
                <div className="rounded-2xl border border-amber-200 bg-amber-500/5 p-4">
                  <h4 className="font-bold text-amber-700 text-sm mb-2 flex items-center gap-1.5">🏠 Tarefa de Casa</h4>
                  <p className="text-gray-600 text-xs">{plano.tarefa_casa}</p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-3">
              ...
              {plano.observacoes && (
                <div className="rounded-2xl border border-gray-200 bg-white p-4">
                  <h4 className="font-bold text-gray-900 text-sm mb-2 flex items-center gap-1.5"><AlertTriangle className="w-3.5 h-3.5 text-amber-600" /> Observações</h4>
                  <p className="text-gray-500 text-xs">{plano.observacoes}</p>
                </div>
              )}
            </div>
          </div>
...
// ─── Histórico de Conteúdos (Professor) ──────────────────────────────────────
...
function HistoricoSection({ apiFetch }: { apiFetch: (u: string, o?: RequestInit) => Promise<Response> }) {
  const [items, setItems] = useState<HistItem[]>([]);
  const [total, setTotal] = useState(0);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [kindFilter, setKindFilter] = useState("");
  const [viewing, setViewing] = useState<HistItem | null>(null);
  ...
  async function load() {
    ...
    try {
      ...
      const r = await apiFetch(`/api/content/history?${params}`);
  ...
  async function openItem(item: HistItem) {
    if (item.kind === "material_premium") {
      const r = await apiFetch(`/api/content/${item.id}`);
      if (!r.ok) { alert("Não foi possível abrir."); return; }
      const d = await r.json();
      const html = d.item?.payload?.html;
      if (html) {
    ...
    if (!r.ok) { alert("Não foi possível abrir."); return; }
    const d = await r.json();
    setViewing(d.item ?? item);
  }

  async function deleteItem(item: HistItem) {
    if (!confirm("Excluir este conteúdo?")) return;
    const r = await apiFetch(`/api/content/${item.id}`, { method: "DELETE" });
    if (r.ok) setItems(prev => prev.filter(i => i.id !== item.id));
  }

  const filtered = search.trim()
    ? items.filter(i => i.title.toLowerCase().includes(search.toLowerCase()) || (i.materia ?? "").toLowerCase().includes(search.toLowerCase()))
    : items;
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-3 mb-2">
        <div className="w-9 h-9 rounded-xl bg-fuchsia-100 flex items-center justify-center">
          <History className="w-5 h-5 text-fuchsia-700" />
        </div>
        <div>
          <h2 className="font-black text-gray-900">Histórico de Conteúdos</h2>
          <p className="text-gray-500 text-xs">Tudo que você gerou — pacotes, provas, pesquisas, materiais premium</p>
        </div>
      </div>

      {/* Filtros */}
      <div className="rounded-2xl border border-gray-200 bg-white p-4 flex flex-col lg:flex-row gap-3">
        <div className="flex-1 relative">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-12 text-center">
          ...
          <p className="text-gray-700 font-bold">Nenhum conteúdo ainda</p>
          <p className="text-gray-500 text-sm mt-1">Gere pacotes, provas e pesquisas para que apareçam aqui.</p>
        </div>
      ) : (
        <>
          <p className="text-xs text-gray-500 font-semibold">{total} {total === 1 ? "item" : "itens"}</p>
          <div className="grid gap-3">
            {filtered.map(item => {
              const meta = HIST_KIND_META[item.kind] ?? HIST_KIND_META.content_package;
              ...
              return (
                <motion.div key={item.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-gray-400 text-xs mt-0.5">{new Date(item.created_at).toLocaleDateString("pt-BR", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}</p>
                  </div>
                  <div className="flex gap-2 flex-shrink-0">
                    ...
                    <button onClick={() => deleteItem(item)}
                      className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-gray-50 hover:bg-red-50 text-gray-400 hover:text-red-600 text-xs font-bold transition-colors">
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
                </motion.div>
          ...
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[85vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="border-b border-gray-200 px-6 py-4 flex items-center justify-between">
              <div className="min-w-0">
                <p className="text-xs text-fuchsia-600 font-bold tracking-wider">{HIST_KIND_META[viewing.kind]?.label ?? viewing.kind}</p>
...
function HistViewer({ item }: { item: HistItem }) {
  ...
  if (data?.titulo || data?.title) {
    return (
      <div className="space-y-4 text-sm">
        <h3 className="text-lg font-black text-gray-900">{data.titulo || data.title}</h3>
        {data.resumo && <p className="text-gray-700">{data.resumo}</p>}
        {data.keyPoints?.length > 0 && (
          <div>
            <h4 className="font-black text-gray-900 mb-2 text-sm">Pontos-chave</h4>
            <ul className="list-disc pl-5 text-gray-700 space-y-1 text-xs">{data.keyPoints.map((k: string, i: number) => <li key={i}>{k}</li>)}</ul>
          </div>
        )}
        {data.questions?.length > 0 && (
          <div className="space-y-3">
            <h4 className="font-black text-gray-900 text-sm">Questões</h4>
        ...
        {data.slides?.length > 0 && (
          <div className="space-y-2">
            <h4 className="font-black text-gray-900 text-sm">Slides</h4>
            {data.slides.map((s: any, i: number) => (
              <div key={i} className="rounded-xl border border-gray-200 bg-gray-50 p-3 text-xs">
                <span className="mr-2">{s.emoji}</span>
                <strong>{s.titulo}</strong>
                {s.items && <ul className="list-disc pl-5 mt-1 space-y-0.5">{s.items.map((it: string, j: number) => <li key={j}>{it}</li>)}</ul>}
                {s.texto && <p className="text-gray-600 mt-1">{s.texto}</p>}
              </div>
            ))}
          </div>
        )}
