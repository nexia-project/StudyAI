    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 flex items-center justify-center p-4">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <Button onClick={() => navigate("/app")} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl">
            Voltar ao início
          </Button>
        </motion.div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center mb-6 shadow-xl shadow-indigo-900/40">
            ...
            {state === "not_logged_in" && (
              <div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-8">
                ...
                <Button
                  ...
                  className="w-full bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl py-3 font-bold text-base">
                  Entrar com minha conta
                </Button>
                <p className="text-center text-slate-500 text-xs mt-4">
                  Ainda não tem conta?{" "}
                  <button onClick={() => { sessionStorage.setItem("auth_return_to", "/professor/login"); navigate("/sign-up"); }}
                    className="text-indigo-400 hover:text-indigo-300">Cadastre-se grátis</button>
                </p>
              </div>
            )}
            ...
            {state === "wrong_role" && (
              <div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-8">
                ...
                <form onSubmit={requestAccess} className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-slate-700 border border-slate-600 rounded-xl px-3 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 text-sm resize-none"
                    />
                  </div>
                  {errMsg && <p className="text-red-400 text-sm">{errMsg}</p>}
                  <Button type="submit" disabled={sending}
                    className="w-full bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl py-3 font-bold">
                ...
                <div className="mt-4 pt-4 border-t border-slate-700 flex items-center gap-2 text-slate-500 text-xs">
                  ...
                  <span>Ou entre em contato: <a href="mailto:contato@study.ia.br" className="text-indigo-400 hover:text-indigo-300">contato@study.ia.br</a></span>
                </div>
              </div>
            )}
          </motion.div>
